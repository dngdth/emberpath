  ...
  return (
    <div
      className={`relative hidden lg:flex flex-col justify-between p-12 overflow-hidden h-full rounded-l-[32px] transition-colors duration-300 ${
        isDark
          ? 'bg-gradient-to-br from-[#0B132B] via-[#0F172A] to-[#0A0F1D] text-slate-100 border-r border-slate-800/80'
          : 'bg-gradient-to-br from-blue-50/70 via-indigo-50/30 to-white text-slate-800 border-r border-slate-200/80'
      }`}
    >
      ...
      <div
        className={`absolute inset-0 pointer-events-none transition-opacity duration-300 ${
          isDark
            ? 'bg-[radial-gradient(#38bdf8_1.5px,transparent_1.5px)] opacity-[0.12]'
            : 'bg-[radial-gradient(#3b82f6_1.5px,transparent_1.5px)] opacity-[0.08]'
        }`}
        style={{ backgroundSize: '24px 24px' }}
      />

      {/* Vibrant Ambient Radiant Glow Orbs (Enhanced for Dark Mode) */}
      <div
        className={`absolute -top-24 -left-24 w-[70%] h-[70%] rounded-full blur-[100px] pointer-events-none transition-all duration-500 ${
          isDark
            ? 'bg-gradient-to-br from-blue-600/30 via-indigo-500/20 to-transparent'
            : 'bg-gradient-to-br from-blue-400/25 via-indigo-300/20 to-transparent'
        }`}
      />
      <div
        className={`absolute -bottom-24 -right-24 w-[70%] h-[70%] rounded-full blur-[100px] pointer-events-none transition-all duration-500 ${
          isDark
            ? 'bg-gradient-to-tl from-orange-500/30 via-amber-500/20 to-transparent'
            : 'bg-gradient-to-tl from-orange-400/25 via-amber-300/20 to-transparent'
        }`}
      />
      <div
        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[55%] h-[55%] rounded-full blur-[120px] pointer-events-none transition-all duration-500 ${
          isDark ? 'bg-blue-500/15' : 'bg-blue-300/15'
        }`}
      />
      ...
      <div className="relative z-10">
        ...
        <h1 className="mt-8 text-3xl font-extrabold tracking-tight leading-tight max-w-md">
          ...
          <span className="block mt-1 bg-gradient-to-r from-orange-500 via-amber-400 to-blue-500 bg-clip-text text-transparent">
            Thông Minh & An Toàn
          </span>
        </h1>
      </div>

      {/* Animated SVG Mockup of Evacuation Path with Glowing Aura */}
      <div className="relative z-10 flex items-center justify-center my-6">
        <div
          className={`w-full max-w-[360px] p-6 rounded-3xl border shadow-2xl backdrop-blur-md transition-all ${
            isDark
              ? 'bg-slate-950/60 border-slate-800/90 shadow-[0_0_45px_rgba(59,130,246,0.18)]'
              : 'bg-white/60 border-slate-200/80 shadow-slate-100'
          }`}
        >
          <svg viewBox="0 0 400 240" className="w-full h-auto">
            ...
            <defs>
              <pattern id="login-grid" width="20" height="20" patternUnits="userSpaceOnUse">
                <circle cx="2" cy="2" r="1" fill={isDark ? '#38bdf8' : '#cbd5e1'} opacity={isDark ? '0.4' : '0.3'} />
              </pattern>
            </defs>
            ...
            <rect x="70" y="55" width="60" height="16" rx="8" fill="#10b981" fillOpacity="0.2" />
            <text x="100" y="66" fill="#10b981" fontSize="8" fontWeight="bold" textAnchor="middle">
              BẮT ĐẦU
            </text>

            <rect x="70" y="175" width="60" height="16" rx="8" fill="#10b981" fillOpacity="0.2" />
