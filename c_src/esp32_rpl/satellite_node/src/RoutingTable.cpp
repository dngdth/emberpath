...
RoutingTable::RoutingTable(uint8_t satelliteId)
    : m_satelliteId(satelliteId), m_parentCount(0), m_myRank(0xFFFF), m_dodagVersion(0), m_hasParent(false), m_parentRoutePathLen(0), m_allowedCount(0) {
    memset(m_parentMac, 0, 6);
}
...
void RoutingTable::updateCandidate(const uint8_t *mac, uint16_t rank, uint8_t version, const uint8_t *routePath, uint8_t routePathLen) {
    unsigned long now = millis();
    int foundIdx = -1;
    ...
    if (foundIdx != -1) {
        m_candidates[foundIdx].rank = rank;
        ...
        if (m_parentCount < MAX_PARENT_CANDIDATES) {
            ...
            m_candidates[m_parentCount].rank = rank;
            ...
            if (rank < m_candidates[MAX_PARENT_CANDIDATES - 1].rank) {
                memcpy(m_candidates[MAX_PARENT_CANDIDATES - 1].mac, mac, 6);
                m_candidates[MAX_PARENT_CANDIDATES - 1].rank = rank;
...
void RoutingTable::sortCandidates() {
    for (int i = 0; i < m_parentCount - 1; i++) {
        for (int j = 0; j < m_parentCount - i - 1; j++) {
            if (m_candidates[j].rank > m_candidates[j+1].rank) {
...
void RoutingTable::syncPrimary() {
    if (m_parentCount > 0) {
        memcpy(m_parentMac, m_candidates[0].mac, 6);
        m_myRank = m_candidates[0].rank;
        m_hasParent = true;
